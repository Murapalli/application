...
def login():
    ...
    if st.button("login"):
        if username=="admin" and password=="cpassword@0015":
...
##=====================================================================================
    ...
    selected_options = st.sidebar.multiselect('Select the Month:', options=Options,default=['AUG','SEP','JUL'])
    ...
    with ch1:
        st.subheader('LY Volumes (MT) by Region')
        fig = px.pie(filtered_df, values = "LY QTY (MT)", names = "Region", template = "plotly_dark")
        fig.update_traces(text = filtered_df["Region"], textposition = "inside")
        st.plotly_chart(fig,use_container_width=True)
    with ch2:
        st.subheader('Bud Volumes (MT) by Region')
        fig = px.pie(filtered_df, values = "Budget Vol(MT)", names = "Region", template = "plotly_dark")
        fig.update_traces(text = filtered_df["Region"], textposition = "inside")
        st.plotly_chart(fig,use_container_width=True)
        
    with ch3:
        st.subheader('CY Volumes (MT) by Region')
    ...
    with chart1:
        st.subheader('LY Volumes (MT) by Product Type')
        fig = px.pie(filtered_df, values = "LY QTY (MT)", names = "Type", template = "plotly_dark")
        fig.update_traces(text = filtered_df["Type"], textposition = "inside")
        st.plotly_chart(fig,use_container_width=True)
    with chart2:
        st.subheader('Bud Volumes (MT) by Product Type')
    ...
    char1, char2, char3 = st.columns((3))
    with char1:
        st.subheader('LY Volumes (MT) by Pack Type')
        fig = px.pie(filtered_df, values = "LY QTY (MT)", names = "Pack Type", template = "plotly_dark")
        fig.update_traces(text = filtered_df["Pack Type"], textposition = "inside")
        st.plotly_chart(fig,use_container_width=True)
    with char2:
        st.subheader('Bud Volumes (MT) by Pack Type')
    ...
    with cht1:
        st.subheader('LY Volumes (MT) by Pack Category')
        fig = px.pie(filtered_df, values = "LY QTY (MT)", names = "Pack Cat", template = "plotly_dark")
        fig.update_traces(text = filtered_df["Pack Cat"], textposition = "inside")
        st.plotly_chart(fig,use_container_width=True)
    with cht2:
        st.subheader('Bud Volumes(MT) by Pack Category')
    ...
    TOP=pd.read_excel("Top customers.xlsx")
    ...
    with top1:
      ...
      TOP2 = TOP.groupby(by = ["Customer name","UNIT"], as_index = False)[["Quantity - kgs","QTY (MT)"]].sum().sort_values('QTY (MT)',ascending=0)
      st.write(TOP2.set_index('Customer name').round(0))
    with top2:
        fig = px.pie(TOP, values = "QTY (MT)", names = "UNIT", template = "gridon")
        fig.update_traces(text = TOP["UNIT"], textposition = "inside")
        st.plotly_chart(fig,use_container_width=True)
